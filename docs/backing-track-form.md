...
At generation time the backing-track engine looks up the preset, verifies an exact
meter match, expands it to an isolated ordinary `GrooveSpec`, validates it through
the groove engine, and renders deterministic Type 1 MIDI.
...
Cycle indexes are zero-based and start at the first musical bar after count-in:
...
`generate_backing_tracks.py` routes manifests through `backing_track_engine.py`, the
composition boundary for preset resolution and arrangement form. Existing manifests
without these fields keep their prior deterministic behavior.
