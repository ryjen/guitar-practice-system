...
COMMANDS: tuple[CommandSpec, ...] = (
    CommandSpec(("discover", "search"), "Search deterministic repository catalogs", MigrationState.NATIVE, native_handler="discover-search"),
    CommandSpec(("schedule", "propose"), "Propose a deterministic v2 practice schedule", MigrationState.NATIVE, native_handler="schedule-propose"),
    CommandSpec(("schedule", "check-approval"), "Check whether a v2 schedule proposal can still be approved", MigrationState.NATIVE, native_handler="schedule-check-approval"),
    CommandSpec(("schedule", "legacy", "propose"), "Compatibility access to the v1 scheduling proposal", MigrationState.DEPRECATED, legacy=LegacyTarget("scripts/scheduling.py", ("propose",))),
    CommandSpec(("schedule", "legacy", "check-approval"), "Compatibility access to v1 schedule approval checks", MigrationState.DEPRECATED, legacy=LegacyTarget("scripts/scheduling.py", ("check-approval",))),
    CommandSpec(("assess", "evaluate"), "Evaluate evidence against deterministic assessment gates", MigrationState.NATIVE, native_handler="assess-evaluate"),
    CommandSpec(("session", "adapt"), "Build a deterministic adaptive-session recommendation", MigrationState.LEGACY, legacy=LegacyTarget("scripts/adaptive_session.py")),
    CommandSpec(("evidence", "feedback"), "Derive deterministic feedback from evidence records", MigrationState.LEGACY, legacy=LegacyTarget("scripts/evidence_feedback.py")),
    CommandSpec(("score", "import"), "Import Guitar Pro or MusicXML into canonical song data", MigrationState.NATIVE, native_handler="score-import"),
    CommandSpec(("score", "tracks"), "Inspect imported score tracks and role classification", MigrationState.NATIVE, native_handler="score-tracks"),
    CommandSpec(("groove", "validate"), "Validate the groove catalog", MigrationState.NATIVE, native_handler="groove-validate"),
    CommandSpec(("groove", "list"), "List groove presets", MigrationState.NATIVE, native_handler="groove-list"),
    CommandSpec(("groove", "show"), "Show one groove preset", MigrationState.NATIVE, native_handler="groove-show"),
    CommandSpec(("progression", "validate"), "Validate the progression catalog", MigrationState.NATIVE, native_handler="progression-validate"),
    CommandSpec(("progression", "list"), "List progression presets", MigrationState.NATIVE, native_handler="progression-list"),
    CommandSpec(("progression", "show"), "Show one progression preset", MigrationState.NATIVE, native_handler="progression-show"),
    CommandSpec(("progression", "resolve"), "Resolve a progression preset in a key", MigrationState.NATIVE, native_handler="progression-resolve"),
    CommandSpec(("progression", "fourths"), "Resolve a progression through the circle of fourths", MigrationState.NATIVE, native_handler="progression-fourths"),
    ...
    CommandSpec(("drums", "export"), "Export full-song drums for a supported hardware target", MigrationState.NATIVE, native_handler="drums-export"),
    ...
    CommandSpec(("validate", "public-boundary"), "Validate the public/private repository boundary", MigrationState.LEGACY, legacy=LegacyTarget("scripts/check_public_boundary.py")),
)
