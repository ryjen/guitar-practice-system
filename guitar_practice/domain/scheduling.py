"""Deterministic scheduling rules over explicit normalized practice state."""
...
from guitar_practice.domain.progression_state import DEVELOPMENT_STATES, STATES

CONTRACT_VERSION = "scheduling/v2"
PROGRESSION_STATES = STATES
...
def validate_snapshot(snapshot: Any) -> dict[str, Any]:
    ...
    for index, projection in enumerate(progression):
        ...
        require_string_list(
            projection.get("plateau_observation_ids"),
            f"progression[{index}].plateau_observation_ids",
        )
    ...
    for index, goal in enumerate(goals):
        ...
        targets = require_string_list(
            goal.get("target_item_ids"),
            f"goals[{index}].target_item_ids",
            allow_empty=False,
        )
    ...
    for index, work in enumerate(active_work):
        ...
        if projection["state"] not in DEVELOPMENT_STATES:
            raise SchedulingError(
                f"active work item {item_id} is not in a development progression state"
            )
        ...
        require_int(
            work.get("min_recovery_hours"),
            f"active_work[{index}].min_recovery_hours",
            0,
            720,
        )
    ...
    for index, rule in enumerate(maintenance_rules):
        ...
        require_int(
            rule.get("min_recovery_hours"),
            f"maintenance_rules[{index}].min_recovery_hours",
            0,
            720,
        )
    ...
    for index, dependency in enumerate(dependencies):
        ...
        requires_id = require_id(
            dependency.get("requires_item_id"),
            f"dependencies[{index}].requires_item_id",
        )
...
def maintenance_due(
    ...
    due_date = parse_date(
        last_verified,
        f"{projection['item_id']}.last_verification_date",
    ) + timedelta(days=rule["interval_days"])
...
def classify(snapshot: dict[str, Any]) -> tuple[list[Candidate], list[dict[str, Any]]]:
    ...
    for item_id in sorted(progression_by_id):
        ...
        if state == "maintained":
            ...
            if rule is None:
                excluded.append(
                    {"item_id": item_id, "reason_codes": ["maintenance-rule-missing"]}
                )
                continue
            if not dependencies_satisfied(item_id, dependencies_by_item, progression_by_id):
                excluded.append(
                    {"item_id": item_id, "reason_codes": ["prerequisite-blocked"]}
                )
                continue
            if projection["weekly_count"] >= max_same_target:
                excluded.append(
                    {"item_id": item_id, "reason_codes": ["weekly-repetition-cap"]}
                )
                continue
            if in_recovery(
                projection,
                rule["min_recovery_hours"],
                generated_at,
                high_load,
            ):
                excluded.append(
                    {"item_id": item_id, "reason_codes": ["recovery-window"]}
                )
                continue
            due_state, overdue_days = maintenance_due(projection, rule, effective)
            if due_state == "not-due":
                excluded.append(
                    {"item_id": item_id, "reason_codes": ["maintenance-not-due"]}
                )
                continue
            candidates.append(
                Candidate(
        ...
        candidates.append(
            Candidate(
                ...
                overdue_days=0,
                reasons=("missed-session-recovery",)
                if work["missed"]
                else ("active-work-priority",),
            )
        )
    ...
    if conflicts:
        ...
        for candidate in candidates:
            if remaining < constraints["minimum_block_minutes"]:
                excluded.append(
                    {"item_id": candidate.item_id, "reason_codes": ["session-time-exhausted"]}
                )
                continue
