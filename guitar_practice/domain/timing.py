"""Pure validation for concrete metronome and tempo realizations."""

from __future__ import annotations

from typing import Any

BEAT_UNITS = frozenset(
    {
        "whole",
        "half",
        "quarter",
...
STRATEGIES = frozenset(
    {"ladder", "pyramid", "burst", "subdivision", "loop-expansion", "sparse-click"}
)
SOURCES = frozenset(
    {
        "session",
...
def validate_timing(realization: dict[str, Any]) -> None:
    ...
    if not isinstance(realization.get("stop_condition"), str) or not realization[
        "stop_condition"
    ].strip():
        raise TimingError("stop_condition is required")
    if not isinstance(realization.get("final_check"), str) or not realization[
        "final_check"
    ].strip():
...
def effective_event_rate(bpm: float, notes_per_beat: float) -> float:
    ...
    return _positive_number(bpm, "bpm") * _positive_number(
        notes_per_beat,
        "notes_per_beat",
    )
