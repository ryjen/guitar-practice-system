"""Pure Type 1 MIDI rendering and validation from explicit manifests."""
...
class ManifestError(ValueError):
    """Raised when a backing-track manifest or MIDI payload is inconsistent."""
...
def conductor_track(manifest: dict[str, Any], bar_ticks: int) -> bytes:
    ...
    events = [
        ...
        TimedEvent(
            0,
            1,
            meta(0x51, int(60_000_000 / tempo).to_bytes(3, "big")),
        ),
        TimedEvent(
            0,
            2,
            meta(0x58, bytes([numerator, denominator_power, 24, 8])),
        ),
        TimedEvent(
            0,
            3,
            meta(0x59, key_signature_payload(manifest["key_signature"])),
        ),
    ...
    for section in manifest["sections"]:
        events.append(
            TimedEvent(bar * bar_ticks, 4, meta(0x06, section["name"].encode()))
        )
        bar += section["bars"]
                ...
                for beat, note in enumerate([root, fifth, root, fifth]):
                    events.extend(
                        note_events(
                            ...
                            note=note,
                            velocity=velocity,
                            start=start + beat * beat_ticks,
                            duration=int(beat_ticks * 0.82),
                        )
                    )
...
def render(manifest: dict[str, Any]) -> bytes:
    """Render a validated manifest to deterministic Type 1 MIDI bytes."""
    ...
    tracks.extend(
        generate_track(track, chords, bar_ticks, beat_ticks)
...
def inspect(data: bytes) -> dict[str, Any]:
    """Inspect Type 1 MIDI bytes without filesystem access."""
...
def validate_rendered(manifest: dict[str, Any], data: bytes) -> dict[str, Any]:
    validate_manifest(manifest)
    report = inspect(data)
    expected_names = ["Conductor"] + [track["name"] for track in manifest["tracks"]]
    if report["track_names"] != expected_names:
        raise ManifestError(
            f"track names differ: {report['track_names']} != {expected_names}"
        )
    expected_markers = ["COUNT-IN"] + [
        section["name"] for section in manifest["sections"]
    ] + ["END"]
    if report["markers"] != expected_markers:
        raise ManifestError(
            f"markers differ: {report['markers']} != {expected_markers}"
        )
    if not all(
        report[key] == 1 for key in ("tempo_events", "meter_events", "key_events")
    ):
        raise ManifestError(
            "conductor metadata must contain one tempo, meter, and key event"
        )
    return report
