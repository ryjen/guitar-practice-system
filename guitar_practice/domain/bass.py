...
KICK_NOTE = groove.GENERAL_MIDI_DRUMS["kick"]
...
def parse_bass(raw: Any) -> BassSpec:
    if not isinstance(raw, dict):
        raise midi.ManifestError("bass must be an object")
    unknown = set(raw) - {"style", "gate_percent", "follow_kick_velocity"}
    if unknown:
        raise midi.ManifestError(f"bass has unsupported fields: {sorted(unknown)}")
    ...
    if not isinstance(follow, bool):
        raise midi.ManifestError("bass.follow_kick_velocity must be boolean")
    if style == "walking" and follow:
        raise midi.ManifestError("walking bass cannot follow kick velocity")
    return BassSpec(
        style=style,
...
def _note_for_style(
    ...
    *,
    ...
    raise midi.ManifestError(f"unsupported kick-locked bass style: {style}")
...
def render_bar(
    ...
    *,
    ...
    tempo_bpm: int,
    base_velocity: int,
    groove_spec: groove.GrooveSpec | None = None,
) -> list[BassHit]:
    if bar_index < 0:
